...
| 2026-07-18 | fix              | `fix(output): copy README/markdown image assets into generated HTML wiki` — `MediaAssetResolver` rewrites local `<img>` src to `html/media/` and copies project files; preprocesses markdown `![](path)` syntax (including bracket-heavy alt text) before render; per-segment category intro rendering; theme-aware dual wordmark in README + portal CSS; 528 tests; `src/output/html/media-assets.ts` added                 | uncommitted           | test ✓ · lint ✓ · format ✓ · coverage ✓ · typecheck ✓ · build ✓                      |
